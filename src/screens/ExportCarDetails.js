...
const ExportCarDetails = ({route, navigation }) => {
  const [vehicleDetails , setvehicleDetails] = useState([
  
])

  const {  datapre , vehicles} = route.params;
  ...
  const [imagesEditable , setimagesEditable] = useState([])
...
const handleRemoveItem = e => {
  const newArr = [...imagesEditable];
  newArr.splice(newArr.findIndex(item => item === e), 1)
  setimagesEditable(newArr)
}
...
>
<Image style={{width:'100%',height: 80,borderRadius:5, resizeMode:'cover'}}  source={{uri: item}} />
{/* <Image
                  style={{width: '100%', height: 60,borderRadius:5, resizeMode:'cover'}}
                  source={require('../images/AFGLogofinal.jpg')}
                />  */}
      ...
      .then((responseJson) => {
        ...
        console.warn( 'response',responseJson);
          if (responseJson.status == AppConstance.API_SUCESSCODE) {
            ...
            console.warn( 'response is ::::::::::',responseJson.data.export);

            setspinner(false)
            

            let exportdata = responseJson.data.export;
            if (exportdata.exportImages != undefined && exportdata.exportImages != undefined) {
              for (let index = 0; index < exportdata.exportImages.length; index++) {
                  const element = exportdata.exportImages[index];
                  images.push('https://admin.afgshipping.com/uploads/' + element.thumbnail)
                  console.log('make is '+element.make);
              }
              setimg(exportdata.exportImages)

            }

            setexportdata(responseJson.data.export)
            setdata(responseJson.data.export.vehicle)
            if(exportdata.container_type == "1"){
                                  setsize('20\'')
                                }else if(exportdata.container_type == "2"){
              
                                  setsize('45\'')
              
                                }else if(exportdata.container_type == "3"){
            ...
            // setExport(responseJson.data.export.vehicle)
...
useEffect(() => {
  ...
  // setInterval(() => {
  AsyncStorage.getItem(AppConstance.USER_ROLE).
  then((value) => {
   AppConstance.USER_ROLE = value
console.log(AppConstance.USER_ROLE+'-3-3-3---------=='+value);
         ...
         >
          <SafeAreaView
            ...
            <FlatList
                      ...
                      data={[...imagesEditable, { plusImage: true }]}
                  ...
                  <View style={{flexDirection:'row',height:50,paddingVertical:5, justifyContent:'center', width:'100%'}}>
                    ...
                    <TouchableOpacity 
                    onPress={()=>{setEditImages(false),setimagesEditable('')}}
                    ...
                    <TouchableOpacity style={{backgroundColor:'lightblue',justifyContent:'center',borderRadius:20,  width:'45%'}}>
...
<Text style={{marginLeft:6,alignSelf:'center'}}>View Export</Text>
</View>
...
<Text style={{color:'black',fontWeight:'bold',paddingVertical:2, fontSize:12,}}>Container No: {exportdata.container_number}</Text>
<Text style={{color:'black',fontWeight:'bold',paddingVertical:2, fontSize:12,}}>AR No: {exportdata.ar_number}</Text>
<Text style={{color:'black',fontWeight:'bold',paddingVertical:2, fontSize:12,}}>Loaded From: {exportdata.port_of_loading}</Text>
...
{AppConstance.USER_ROLE == '0' ?
  ...
  onPress={()=> navigation.navigate('ExportImageEdit',{images:img,exportid:datapre,})}
  >
    <Image  style={{width:30 , height:30 , alignSelf:'center', }}   source={require('../images/Unknown.jpg')} />

  {/* <Text style={{color:'grey',paddingVertical:2,alignSelf:'center', textAlign:'center', fontSize:14,}}>Edit Or Upload Images</Text> */}
...
<Text style={{textAlign:'left',width:'50%', color:'black',fontSize:15}}>{exportdata.port_of_loading}</Text>
</View>
...
<Text style={{textAlign:'left',width:'50%', color:'black',fontSize:15}}>{exportdata.port_of_discharge}</Text>
</View>


<View style={{flexDirection:'row',paddingVertical:5,}}>
<Text style={{textAlign:'left',width:'50%', color:'black',fontSize:15}}>Container no</Text>
<Text style={{textAlign:'left',width:'50%', color:'black',fontSize:15}}>{exportdata.container_number}</Text>
</View>
...
<Text style={{textAlign:'left',width:'50%', color:'black',fontSize:15}}>{exportdata.booking_number}</Text>
</View>
...
<Text style={{textAlign:'left',width:'50%', color:'black',fontSize:15}}>{exportdata.eta}</Text>
</View>
